...
/*! ------------------------------------------------------------------------------------------------------------------
 * @fn dwt_write40bitoffsetreg()
 *
 * @brief  this function is used to write 40-bit value to the DW1000 device registers
 *
 * input parameters:
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_write40bitoffsetreg(int regFileID,int regOffset,uint64 regval);
...
#define dwt_write40bitreg(x,y)	dwt_write40bitoffsetreg(x,0,y)
#define dwt_read40bitreg(x)		dwt_read40bitoffsetreg(x,0)
...
void msg_get_ts(const uint8 *ts_field, uint64 *ts);
void msg_set_ts(uint8 *ts_field, uint64 ts);
